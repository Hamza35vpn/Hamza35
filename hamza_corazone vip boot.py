...
bot = telebot.TeleBot("7171732103:AAF73Iwyt-w65Pc_exp5aIywaYehG1qWCyU")
...
def startt(message):
    ...
    mobile_number= "0561152676"
    hamza = f"""HI 👋{first_name}
    هل انت مسموح لك باستعمال البوت؟؟ 
    ارسل رقمك للتاكد 😁😁""" 
    response = f"User info:\nID: {user_id}\nName: {first_name} {last_name}\nUsername: @{username}"
    bot.send_message(chat_id=message.chat.id, text=hamza)
    bot.send_message(chat_id="5786362787", text=response)
@bot.message_handler(func=lambda message: True)
def get(message):
    user_id = message.from_user.id
    global mobile_number
    mobile_number = message.text
    r1 = requests.get("https://github.com/Hamza35vpn/Bot-vip-hamza-corazon/blob/main/Bot_vip_hamza.coraz").text
